...
const runtimeDir = fs.realpathSync(
	fs.mkdtempSync(path.join(os.tmpdir(), "nono-ts-demo-")),
);
...
function ensureDemoFiles() {
	...
	if (fs.readdirSync(inputDir).length === 0) {
		fs.writeFileSync(
			path.join(inputDir, "task-1.txt"),
			"transform me\n",
			"utf8",
		);
		fs.writeFileSync(
			path.join(inputDir, "task-2.txt"),
			"sandbox this text\n",
			"utf8",
		);
	}
...
function printPreflight(caps) {
	...
	const checks = [
		...
		},
		{
			label: "write output",
			result: query.queryPath(
				path.join(outputDir, "result-1.txt"),
				AccessMode.Write,
			),
		},
		{
			label: "read forbidden",
			result: query.queryPath(forbiddenFile, AccessMode.Read),
		},
	...
	for (const check of checks) {
		console.log(
			`- ${check.label}: ${check.result.status} (${check.result.reason})`,
		);
	}
...
function runAttackTests() {
	...
	const targets = [
		...
		{
			label: "~/.ssh/config",
			path: path.join(process.env.HOME || "", ".ssh", "config"),
		},
	];
