...
describe('QueryContext policy audit', () => {
  ...
  it('allows reading inside pluginDir', () => {
    expect(ctx.queryPath(join(pluginDir, 'init.js'), AccessMode.Read).status).toBe('allowed');
  });

  it('allows writing inside outputDir', () => {
    expect(ctx.queryPath(join(outputDir, 'result.json'), AccessMode.Write).status).toBe('allowed');
  });
  ...
  it('denies writing inside pluginDir', () => {
    expect(ctx.queryPath(join(pluginDir, 'evil.js'), AccessMode.Write).status).toBe('denied');
  });

  it('denies reading inside secretDir', () => {
    expect(ctx.queryPath(join(secretDir, 'key.pem'), AccessMode.Read).status).toBe('denied');
  });
