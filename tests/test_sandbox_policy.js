...
const tmpBase = fs.realpathSync(
	fs.mkdtempSync(path.join(os.tmpdir(), "nono-test-")),
);
...
for (const fc of fsCaps) {
	const kind = fc.isFile ? `${YELLOW}file${RESET}` : `${CYAN}dir ${RESET}`;
...
assert(
	fsCaps.length === 3,
	"expected 3 fs capabilities (pluginDir, outputDir, configFile)",
);
...
denied(
	`Write plugins/evil.js`,
	`granted=${q3.granted}, requested=${q3.requested}`,
);
...
assert(
	state.netBlocked === true,
	"serialized state should reflect network blocked",
);
...
assert(
	restoredCaps.fsCapabilities().length === 3,
	"restored caps should have 3 fs entries",
);

const ctx2 = new QueryContext(restoredCaps);
assert(
	ctx2.queryPath(path.join(pluginDir, "init.js"), AccessMode.Read).status ===
		"allowed",
	"restored policy should still allow reading pluginDir",
);
assert(
	ctx2.queryPath(path.join(secretDir, "key.pem"), AccessMode.Read).status ===
		"denied",
	"restored policy should still deny reading secretDir",
);
assert(
	ctx2.queryNetwork().status === "denied",
...
if (isSupported()) {
	...
	console.log(
		`  ${DIM}permanently restrict this process to the granted capabilities.${RESET}`,
	);
}
