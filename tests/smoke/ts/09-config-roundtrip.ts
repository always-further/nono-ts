...
console.log("nono-ts example: config roundtrip (TypeScript)");
...
function capsFromConfig(config: {
	paths: { path: string; access: string; isFile?: boolean }[];
...
console.log(
	`- original: ${originalDecision.status} (${originalDecision.reason})`,
);
console.log(
	`- restored: ${restoredDecision.status} (${restoredDecision.reason})`,
);
