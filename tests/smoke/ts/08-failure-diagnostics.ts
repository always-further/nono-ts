...
console.log("nono-ts example: failure diagnostics (TypeScript)");
...
console.log(
	`Preflight denied path: ${deniedQuery.status} (${deniedQuery.reason})`,
);
...
try {
	...
	console.log(
		`Runtime denial captured: ${e?.code || "UNKNOWN"} - ${e?.message || String(error)}`,
	);
