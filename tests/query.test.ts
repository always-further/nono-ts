...
import { CapabilitySet, QueryContext, AccessMode } from '../index.js';
...
describe('QueryContext', () => {
  it('allows a path that was granted', () => {
    ...
    expect(ctx.queryPath('/tmp/test.txt', AccessMode.Read).status).toBe('allowed');
  });
  ...
  it('denies write when only read was granted', () => {
    ...
    const ctx = new QueryContext(caps);
    expect(ctx.queryPath('/tmp/out.txt', AccessMode.Write).status).toBe('denied');
  });
